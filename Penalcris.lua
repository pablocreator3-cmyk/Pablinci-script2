...
local Window = Rayfield:CreateWindow({
   ...
   LoadingTitle = "Activando Modo Asesino...",
...
-- SECCIÓN FARM (Lo de siempre, intacto)
         ...
         while getgenv().fastPunch do
            ...
            if tool then
               ...
               for _, anim in pairs(char.Humanoid:GetPlayingAnimationTracks()) do
                   anim:AdjustSpeed(4) -- Velocidad máxima
               end
            end
            task.wait(0.001)
         end
...
-- SECCIÓN COMBATE (NUEVA)
CombatTab:CreateSection("PVP / Asesinato")

CombatTab:CreateToggle({
   Name = "Auto-Kill Jugadores (TP + Kill)",
   CurrentValue = false,
   Flag = "KillTgl",
   Callback = function(Value)
      ...
      task.spawn(function()
         while getgenv().autoKill do
            pcall(function()
                ...
                -- Si hay alguien cerca, ir por él
                if closestPlayer then
                    -- Te teletransporta detrás del enemigo
                    localPlayer.Character.HumanoidRootPart.CFrame = closestPlayer.Character.HumanoidRootPart.CFrame * CFrame.new(0, 0, 3)
            ...
            task.wait(0.1) -- Velocidad de persecución
         end
      end)
   end,
})
...
Rayfield:Notify({
   Title = "pablo_DTH Hub Asesino",
   Content = "Modo Auto-Kill listo. ¡Cuidado con el baneo!",
   Duration = 5,
   Image = 4483345998,
})
